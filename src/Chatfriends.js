...
export default function Chatfriends() {
  ...
  return (
      <div className=' border-l-2 border-gray-400 border-opacity-20 bg-zinc-100  w-1/2 flex flex-col pl-2  '>
          <Search />
          <div className=' w-full flex  items-center flex-col items-cent h-full  mt-5 flex-nowrap overflow-scroll'>
              {users.map((user) => {
             
        return     <div onClick={()=>handleClick(user.id)}  className=' cursor-pointer hover:bg-zinc-200 my-2 flex flex-row w-3/5 shadow-md  h-20 flex-shrink-0'>
